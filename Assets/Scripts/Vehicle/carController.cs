...
public class carController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        fwdInput = Keyboard.current.wKey.isPressed ? 1.0f : 0.0f;
        backInput = Keyboard.current.sKey.isPressed ? 1.0f : 0.0f;
    ...
    void moveCar()
    {
        if (carDriveType == driveType.AllWheelDrive)
        {
            ...
            wheelColliders[0].motorTorque = wheelColliders[1].motorTorque =
                wheelColliders[2].motorTorque = wheelColliders[3].motorTorque = outputTorque;
        }
    ...
    void ackermanSteering()
    {
        ...
        horizontalInput = Keyboard.current.dKey.isPressed ? 1.0f : 0.0f;
        horizontalInput -= Keyboard.current.aKey.isPressed ? 1.0f : 0.0f;
    ...
    void adjustTraction()
    {
        if (!Keyboard.current.spaceKey.isPressed)
        {
            ...
            forwardFriction.extremumValue =
                forwardFriction.asymptoteValue = ((currSpeed * frictionMultiplier) / 300) + 1;
            sidewaysFriction.extremumValue =
                sidewaysFriction.asymptoteValue = ((currSpeed * frictionMultiplier) / 300) + 1;
        ...
        else if (Keyboard.current.spaceKey.isPressed)
        {
            ...
            sidewaysFriction.extremumValue = sidewaysFriction.asymptoteValue =
                Mathf.SmoothDamp(sidewaysFriction.asymptoteValue, handBrakeFriction, ref velocity,
                    0.05f * Time.deltaTime);
            forwardFriction.extremumValue = forwardFriction.asymptoteValue =
                Mathf.SmoothDamp(forwardFriction.asymptoteValue, handBrakeFriction, ref velocity,
                    0.05f * Time.deltaTime);
    ...
    void checkWheelSpin()
    {
        ...
        for (int i = 2; i < 4; i++)
        {
            ...
            if (wheelHit.sidewaysSlip < 0)
                tempo = (1 + -hInp) * Mathf.Abs(wheelHit.sidewaysSlip * handBrakeFrictionMultiplier);
    ...
    float checkForTraction(float EE)
    {
        for (int i = 2; i < 4; i++)
        {
            ...
            if ((wheelHit.sidewaysSlip > .3f || wheelHit.sidewaysSlip < -0.3f) && !Keyboard.current.spaceKey.isPressed)
            {
